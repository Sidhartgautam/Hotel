...
class WebsiteFAQCreateView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if parent_id:
            try:
                parent_faq = MoreLivingFAQ.objects.get(id=parent_id)

                # Check if the parent FAQ is already a reply
                if parent_faq.parent is not None:
                    return PrepareResponse(
                        success=False, 
                        message="Replies cannot have further replies"
                    ).send(400)
            except MoreLivingFAQ.DoesNotExist:
                return PrepareResponse(success=False, message="Parent FAQ not found").send(404)
        else:
            parent_faq = None
        ...
        serializer = self.get_serializer(data=request.data)
        if serializer.is_valid():
            serializer.save(user=request.user, parent=parent_faq)
            return PrepareResponse(success=True, data=serializer.data, message="Website FAQ added").send(201)
...
class WebsiteFAQListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return MoreLivingFAQ.objects.filter(parent__isnull=True).prefetch_related('replies')
